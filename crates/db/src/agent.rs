...
pub async fn create(
    ...
    let row = sqlx::query_as::<_, AgentRow>(
        "INSERT INTO agents (name, description, llm_connector_id, system_prompt, temperature, max_tokens, container_enabled, container_config)
...
pub async fn list_all(db: &Db) -> Result<Vec<AgentRow>, DbError> {
    ...
    let row = sqlx::query_as::<_, AgentRow>(
        "UPDATE agents
         SET name = $2, description = $3, status = $4, llm_connector_id = $5,
             ...
             container_enabled = $9, container_config = $10, updated_at = now()
         WHERE id = $1
         RETURNING *",
    )
    .bind(id)
