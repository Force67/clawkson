...
async fn create(
    ...
    if existing.is_empty() {
        let _ = clawkson_db::settings::update(&state.db, Some(Some(row.id)), None, None, None, None, None, None, None).await;
    }
        ...
        Ok(true) => {
            ...
            if let Ok(settings) = clawkson_db::settings::get(&state.db).await {
                if settings.default_llm_connector_id == Some(id) {
                    ...
                    let _ = clawkson_db::settings::update(&state.db, Some(next), None, None, None, None, None, None, None).await;
                }
            }
